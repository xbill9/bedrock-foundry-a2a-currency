...
Set the A2A peer's endpoint variable — ``CURRENCY_A2A_ENDPOINT`` for the ADK
agent, ``CURRENCY_FOUNDRY_A2A_ENDPOINT`` for the Foundry agent, selected with
``CURRENCY_A2A_PEER`` — and optionally ``CURRENCY_RATE_PROVIDER=frankfurter``
with ``CURRENCY_RATE_TRANSPORT=mcp-stdio``, to exercise the live protocol
stack.
"""
