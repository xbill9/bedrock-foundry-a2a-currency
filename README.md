# Bedrock AgentCore Cross-Cloud Currency Agent

A buildable interoperability lab in which a Strands Agents coordinator, hosted
on Amazon Bedrock AgentCore Runtime, verifies currency conversions by
combining:

- a live exchange-rate tool exposed through MCP;
- a remote currency agent exposed through A2A v1.0 — either a Google ADK agent
  on Cloud Run or a Microsoft Foundry hosted agent, selectable per run; and
...
and cross-framework portability. Two remote peers behind one client let the
same coordinator and the same 38 cases compare AWS→GCP against AWS→Azure, and
the same domain core previously ran with a Microsoft Foundry–hosted
coordinator (results retained in `evaluations/results/`), so the coordinator
side is comparable like-for-like too.
...
CLI / test runner
       |
Bedrock AgentCore Runtime hosted agent
Strands Agents on a Bedrock model (Python)
       |
       +-- MCP --> exchange-rate server
       |
       +-- A2A v1.0 --> Google ADK currency agent (Cloud Run, Gemini)
       |               anonymous, well-known agent card
       |
       +-- A2A v1.0 --> Microsoft Foundry currency agent (Azure, gpt-5-mini)
       |               Entra bearer token, agentCard/v1.0, JSONRPC only
       |
       +-- OpenTelemetry traces + evaluation results
```

One peer is selected per run with `CURRENCY_A2A_PEER` (or `--a2a-peer`).
Everything vendor-specific about the two legs lives in
`coordinator/a2a_peers.py`.

## Research questions

1. Can an AgentCore-hosted Strands agent discover and invoke both a Google ADK
   agent and a Microsoft Foundry agent through an A2A agent card without
...
5. What does authenticating across clouds actually cost in engineering terms —
   an AWS-hosted caller has no Azure identity, and Foundry accepts no keys.
...
- two remote currency agents (Google ADK on Cloud Run, Microsoft Foundry);
- one MCP exchange-rate server;
- one AgentCore Runtime deployment;
- AWS IAM/SigV4 authentication via the standard credential chain, and Entra
  service-principal authentication for the Foundry peer;
...
foundry_agent/     Microsoft Foundry hosted agent exposed over A2A v1.0
...
## Deploy the Foundry A2A peer

Requires `az login` + `azd auth login` and an identity with the Foundry
data-plane role. One command syncs the bundle, provisions, deploys, and enables
incoming A2A:
...
Then give the coordinator an Azure identity — it runs on AWS and Foundry
accepts no keys — and point it at the new peer:

```bash
./infra/configure_azure_secret.sh          # service principal -> AWS Secrets Manager
./infra/point_coordinator_at_foundry.sh    # rewrite agentcore.json, redeploy, smoke-test
```

See `foundry_agent/README.md` for what Foundry's preview A2A endpoint requires,
and `infra/README.md` for the role assignments.
...
| `a2a_only` | AgentCore coordinator delegates to the A2A peer | Measure remote-agent behavior |
